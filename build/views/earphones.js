...
// Generate earphones shop
let generateEarphones = () => {
    shopContainer.innerHTML = earphones.map(product => {
        ...
        return `
            <div class="product-card">
                ...
                <div class="buttons-plus-minus">
                    <i onclick="decrementEarphones(${product.id})" class="bi bi-dash-lg"></i>
                    <div class="quantity" id="quantity-${product.id}">
                        ${search.item || 0}
                    </div>
                    <i onclick="incrementEarphones(${product.id})" class="bi bi-plus-lg"></i>
                </div>
            </div>
        `;
    }).join("");
